# How to run Fleunce on your iPhone
...
Both paths remain ignored by Git. If both files exist, compare them privately and keep the intended team and account settings in the new file; do not publish their contents. Keep the development team and bundle identifier used by your installed app. If you previously changed the identifier directly in Xcode, confirm that change is present in the relocated project before running it. Open `apps/ios/Fleunce.xcodeproj` for future builds. Moving the project does not move or erase the learning data or Keychain entries already on your iPhone.

## Install

1. Download the [repository ZIP](https://github.com/Chuloo/fleunce/archive/refs/heads/main.zip) and extract it, or clone [Chuloo/fleunce](https://github.com/Chuloo/fleunce). Open `apps/ios/Fleunce.xcodeproj`. Allow Xcode to resolve the pinned WebRTC package.
2. Select the blue **Fleunce** project in the navigator. Under **Targets**, choose **Fleunce**, then open **Signing & Capabilities**.
3. Enable **Automatically manage signing** and choose your Apple team. For a fork, set a unique bundle identifier, such as `com.yourname.fleunce`. Do not change an existing installation’s identifier when refreshing it.
...
6. In Xcode’s toolbar, select the **Fleunce** scheme and your iPhone as the destination. Click **Run** or press **⌘R**. If macOS requests access to the signing key, allow Xcode to use it.
...
Fleunce should greet you aloud. Disconnect the cable and confirm a short conversation over Wi-Fi or cellular.
...
| No microphone input | Enable Fleunce under iPhone Settings → Privacy & Security → Microphone. |
| Sound uses the wrong output | Check iOS’s current audio destination; disconnect an unwanted Bluetooth route. |
| A connection fails on cellular | Confirm Fleunce is allowed to use cellular data and retry with a stable connection. |
