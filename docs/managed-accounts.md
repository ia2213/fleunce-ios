...
```xcconfig
FLEUNCE_MANAGED_ACCOUNTS_ENABLED = YES
FLEUNCE_MANAGED_API_URL = https:/$()/api.fleunce.chat
FLEUNCE_GOOGLE_SIGN_IN_ENABLED = YES
FLEUNCE_GOOGLE_CLIENT_ID = 1034240936303-gernpir75rp4uvc8m5imrhrs18npma5n.apps.googleusercontent.com
FLEUNCE_GOOGLE_CALLBACK_SCHEME = com.googleusercontent.apps.1034240936303-gernpir75rp4uvc8m5imrhrs18npma5n
FLEUNCE_APPLE_SIGN_IN_ENABLED = NO
FLEUNCE_APPLE_SWIFT_FLAGS =
FLEUNCE_APPLE_ENTITLEMENTS =
```

These values identify Fleunce's public iOS client in Google Cloud project `fleunce-508413`, for bundle `no.william.fleunce`. For another backend or bundle ID, substitute its origin and matching OAuth client. Keep the `https:/$()/` syntax in an xcconfig file so `//` is not treated as a comment.
...
6. Complete Google sign-in on the phone. Confirm the signed-in provider and verified email, when supplied, match the intended account. The app fetches `GET /v1/account` and validates its account ID against the active Fleunce session. It does not fetch or display a wallet in this release.
...
```xcconfig
FLEUNCE_APPLE_SIGN_IN_ENABLED = YES
FLEUNCE_APPLE_SWIFT_FLAGS = FLEUNCE_SIGN_IN_WITH_APPLE
FLEUNCE_APPLE_ENTITLEMENTS = App/SignInWithApple.entitlements
```
...
To use Apple alone, set `FLEUNCE_GOOGLE_SIGN_IN_ENABLED = NO`. Each provider is gated independently. Adding a provider preserves existing Fleunce sessions for the same backend origin and app bundle ID.
...
- Relaunch and confirm the Fleunce session persists in its separate Keychain record. Only the Fleunce session, account ID, provider, expiry and backend/app scope are saved there. It uses `WhenUnlockedThisDeviceOnly`, does not sync through iCloud Keychain, and does not access the OpenAI key.
- Sign out and verify the old server session is rejected. If offline, confirm the message distinguishes local sign-out from server revocation; Fleunce sessions expire within 24 hours.
