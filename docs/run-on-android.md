# Install and use Fleunce on Android

## Personal installation

You need Android 8.0 or later and an OpenAI project key with access to the models Fleunce uses. A ChatGPT subscription does not include API credit.
...
3. Run:
   ...
   adb shell am start -n chat.fleunce.android/chat.fleunce.MainActivity
   ```

`-r` updates the app and keeps its data. Updates need the same application ID (`chat.fleunce.android`) and the same signing key; do not uninstall the app if you want to keep your history. USB or wireless debugging is only needed to install and test from a computer, not for everyday use. The debug APK is for personal installation, not for Google Play.
...
Fleunce speaks the interface language of your phone: Spanish on a Spanish phone, English otherwise. That is separate from the language you practise and the language you read meanings in, which you choose next.
...
Fleunce needs the internet to talk, translate and search. History and vocabulary are available offline. Requests are billed to your OpenAI project; **Settings** shows recorded voice time and a voice cost estimate. The app's time limit is not a billing cap.

## Move data from iPhone

On iPhone, export a learning backup from Settings and transfer the JSON file any way you like. On Android, choose **Settings → Import learning backup**. Fleunce v1 and v2 backups up to 30 MB are accepted. Conversations that are not already saved are added, and the Android settings are kept. The key is not transferred: enter it again on the phone.

Android exports use the same format, and iPhone can import them. There is no automatic sync or Fleunce account.
...
The interface tests install as a separate app, `chat.fleunce.android.uitest`, so they never read or change the data of your Fleunce installation. They do not need an API key and do not call OpenAI.
...
- **Microphone blocked:** Android Settings → Apps → Fleunce → Permissions → Microphone. You can also type.
- **Sound plays on the speaker with a Bluetooth headset:** connect the headset before starting the conversation. Fleunce chooses a connected headset on Android 12 or later; on Android 8–11 it uses the speaker.
- **Key rejected or usage limit reached:** check the key, model access and project limits in your OpenAI account. Fleunce never needs you to share your key with anyone.
- **Switching to another app:** Fleunce ends a voice conversation to release the microphone. Come back and start another; history is kept.
- **Backup error:** make sure it is a complete Fleunce file under 30 MB. The importer rejects a file before merging invalid records.
