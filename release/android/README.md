...
The owner-approved permanent package is `chat.fleunce.android`. Play registration and signing are separate release steps. `versionCode` starts at 1; each later upload must use a higher value. A debug installation is not a Play-signed release.
