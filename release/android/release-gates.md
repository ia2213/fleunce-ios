...
- [ ] Android's launcher image is byte-identical to the canonical iOS AppIcon, and resource references select that image. Installed launcher masks/scale and the in-app Fleunce mark match the iOS reference. Play icon and feature artwork derive from the same approved mark, without redrawing or recoloring it.
