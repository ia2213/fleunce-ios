...
work = Path(tempfile.mkdtemp(prefix='fleunce-play-capture-'))
adb = str(Path(sdk)/'platform-tools/adb')
aapt2 = Path(sdk)/'build-tools/36.0.0/aapt2'
for apk, expected in [(app_apk, 'chat.fleunce.android.uitest'), (test_apk, 'chat.fleunce.android.uitest.test')]:
    if not apk.is_file():
...
evidence = {'recordedAtUTC': datetime.datetime.now(datetime.timezone.utc).isoformat(),
            ...
            'fixture': {'path': 'apps/android/app/src/androidTest/java/chat/fleunce/PlayStoreCaptureTest.kt',
                        'sha256': digest(root/'apps/android/app/src/androidTest/java/chat/fleunce/PlayStoreCaptureTest.kt'),
...
try:
    ...
    result = device('shell','am','instrument','-w','-e','class',
        'chat.fleunce.PlayStoreCaptureTest,chat.fleunce.PlayFeatureGraphicTest,chat.fleunce.LargeTypeOnboardingTest',
        'chat.fleunce.android.uitest.test/androidx.test.runner.AndroidJUnitRunner', check=False, timeout=120)
    ...
    for name in ['01-greeting.png','02-conversation.png','03-themes.png','04-words.png','05-languages.png','06-settings.png','feature-graphic.png']:
        ...
        png = device('exec-out','run-as','chat.fleunce.android.uitest','cat','files/play-store/'+name).stdout
    ...
    device('shell','am','start','--user','0','-n','chat.fleunce.android/chat.fleunce.MainActivity',check=False)
