...
final class FleunceUITests: XCTestCase {
    ...
    func testEndNoticeKeepsReasonButClearsStaleHelp() {
        for inactivity in [true, false] {
            ...
            app.launch()
            let expected = inactivity ? "Fleunce ended this quiet session to avoid running up usage." : "Conversation saved. Final voice usage is unconfirmed."
            XCTAssertTrue(app.staticTexts[expected].waitForExistence(timeout: 10))
            XCTAssertFalse(app.staticTexts["Fleunce will make that a little simpler."].exists)
        }
    }
    ...
    func testQuietSessionClosesAndPreservesItsExplanation() {
        ...
        app.launch()
        let ended = app.staticTexts["Fleunce ended this quiet session to avoid running up usage."]
