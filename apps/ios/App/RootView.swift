            ...
            ScrollView {
                VStack(spacing: 0) {
                    Text(coordinator.selectedTheme?.title ?? coordinator.language.talkTitle)
                        .font(.system(.caption, design: .rounded, weight: .medium)).foregroundStyle(FleunceColor.secondary)
                        .padding(.horizontal, 14).padding(.vertical, 9).background(FleunceColor.butter.opacity(0.58), in: Capsule()).padding(.top, 12)
                    Spacer(minLength: 8)
                    FleunceOrb(energy: max(coordinator.outputLevel, coordinator.inputLevel * 0.45), listening: coordinator.state == .active && !coordinator.isMuted, active: coordinator.state != .closing)
                    ...
                    .font(.system(.caption, design: .rounded)).foregroundStyle(FleunceColor.secondary)
                    ...
                    Spacer(minLength: 12)
                    controls
                    Text(coordinator.microphoneLabel).font(.caption2).foregroundStyle(FleunceColor.secondary).padding(.top, 10)
                    ...
                    if let notice = coordinator.notice {
                        Text(notice).font(.footnote).foregroundStyle(FleunceColor.secondary).multilineTextAlignment(.center).padding(.bottom, 12)
                    }
        ...
        VStack(spacing: 12) {
            Text(linkedCaption).font(.system(coordinator.assistantPassage == nil ? .largeTitle : .title2, design: .rounded, weight: .medium))
                ...
                .environment(\.openURL, OpenURLAction { url in
                    guard url.scheme == "fleunce-word", let components = URLComponents(url: url, resolvingAgainstBaseURL: false), let word = components.queryItems?.first?.value else { return .discarded }
            ...
            if coordinator.store.preferences.meaningVisible {
                Text(coordinator.assistantPassage == nil ? MeaningLanguages.greeting(in: coordinator.store.preferences.meaningLanguage) : !coordinator.meaning.isEmpty ? coordinator.meaning : coordinator.translating ? "Finding the meaning…" : "")
                    .font(.subheadline).foregroundStyle(FleunceColor.secondary).multilineTextAlignment(.center)
                    .accessibilityIdentifier("meaning-caption")
                if let error = coordinator.meaningError {
                    VStack(spacing: 6) {
                        Text(error).foregroundStyle(FleunceColor.secondary)
            ...
            if let user = coordinator.userPassage {
                ...
                }.foregroundStyle(FleunceColor.secondary).multilineTextAlignment(.center).padding(.top, 3)
            }
            if coordinator.working { ProgressView("Checking that for you…").font(.caption).tint(FleunceColor.secondary) }
    ...
    private var linkedCaption: AttributedString {
        ...
        for segment in CaptionWords.segments(coordinator.caption, languageID: coordinator.language.id) {
            ...
            if coordinator.assistantPassage != nil, let word = segment.lookup {
                var components = URLComponents(); components.scheme = "fleunce-word"; components.host = "lookup"
                components.queryItems = [URLQueryItem(name: "word", value: word)]
                part.link = components.url
            }
            part.foregroundColor = FleunceColor.ink; result.append(part)
        }
        return result
    }
        ...
        HStack(alignment: .center, spacing: 27) {
            Button { coordinator.toggleMeaning() } label: {
                VStack(spacing: 6) {
                    Image(systemName: coordinator.store.preferences.meaningVisible ? "captions.bubble.fill" : "captions.bubble")
                        .frame(width: 48, height: 48).modifier(SoftGlass(tint: coordinator.store.preferences.meaningVisible ? FleunceColor.butter.opacity(0.7) : .white.opacity(0.4)))
            ...
            Button {
                ...
                ZStack {
                    ...
                    if coordinator.state == .connecting || coordinator.state == .closing { ProgressView().tint(FleunceColor.ink) }
                ...
                }.frame(width: 76, height: 76).shadow(color: FleunceColor.orange.opacity(0.25), radius: 10, y: 6)
    ...
    var body: some View {
        NavigationStack {
            VStack(alignment: .leading, spacing: 20) {
                ...
                Text(item.sentence).font(.title3).foregroundStyle(FleunceColor.secondary)
                if let explanation { Text(explanation).font(.body).textSelection(.enabled) }
                else if let error { Text(error).foregroundStyle(FleunceColor.secondary) }
                else { ProgressView("Finding the meaning…") }
                Spacer()
            }.padding(28).frame(maxWidth: .infinity, alignment: .leading).background(FleunceColor.cream)
        ...
        NavigationStack {
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                if let error = coordinator.typedReplyError {
                    Text(error).font(.footnote).foregroundStyle(FleunceColor.secondary).fixedSize(horizontal: false, vertical: true).accessibilityIdentifier("typed-reply-error")
                }
                Button { sending = true; Task { let ok = await coordinator.sendTyped(text); sending = false; if ok { dismiss() } } } label: {
                    HStack { Text(sending ? "Sending…" : "Send reply").fixedSize(horizontal: false, vertical: true); Spacer(); Image(systemName: "arrow.up") }.padding(18).background(FleunceColor.orange, in: Capsule())
                ...
                Spacer()
            }.padding(26).frame(maxWidth: .infinity, alignment: .leading).foregroundStyle(FleunceColor.ink)
            }.accessibilityIdentifier("typed-reply-scroll").background(FleunceColor.cream)
