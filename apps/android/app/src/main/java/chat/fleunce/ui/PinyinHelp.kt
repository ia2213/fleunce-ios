...
import chat.fleunce.core.MandarinPinyin
...
fun PinyinHelp(text: String) {
    ...
    Column(Modifier.fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(2.dp)) {
        TextButton(onClick = { expanded = !expanded }, modifier = Modifier.testTag("pinyin-toggle"),
            colors = ButtonDefaults.textButtonColors(contentColor = FleunceColors.Secondary),
            contentPadding = PaddingValues(horizontal = 8.dp)) {
            FleunceIcon(FleunceSymbol.ChevronDown, Modifier.size(12.dp).rotate(if (expanded) 180f else 0f))
        ...
        if (expanded) SelectionContainer {
            Text(displayed, color = FleunceColors.Secondary, textAlign = TextAlign.Center, style = MaterialTheme.typography.bodyMedium, modifier = Modifier.testTag("pinyin-reading"))
        }
    }
}
