    ...
    ModalBottomSheet(onDismissRequest = onDismiss, containerColor = FleunceColors.Cream,
        ...
        Column(Modifier.fillMaxWidth().testTag("word-lookup-sheet")) {
            Row(Modifier.fillMaxWidth().padding(horizontal = 26.dp), verticalAlignment = Alignment.CenterVertically) {
                ...
                SoftRoundButton(FleunceSymbol.Close, stringResource(R.string.common_close), onDismiss,
            ...
            Column(Modifier.fillMaxWidth().weight(1f, fill = false).verticalScroll(rememberScrollState())
                ...
                SelectionContainer {
                    Text(sentence, style = MaterialTheme.typography.titleLarge, color = FleunceColors.Secondary,
                ...
                when {
                    ...
                    error != null -> Text(error, color = FleunceColors.Secondary, modifier = Modifier.testTag("word-lookup-error"))
                    loading -> Row(horizontalArrangement = Arrangement.spacedBy(10.dp), verticalAlignment = Alignment.CenterVertically) {
                        CircularProgressIndicator(Modifier.size(18.dp), strokeWidth = 2.dp)
                        Text(stringResource(R.string.talk_meaning_loading), color = FleunceColors.Secondary)
                    }
                }
            }
        }
