...
import chat.fleunce.FleunceViewModel
import chat.fleunce.R
import chat.fleunce.core.LanguageRegistry
import chat.fleunce.core.MeaningLanguages
import chat.fleunce.core.Passage
import chat.fleunce.core.SessionRecord
import chat.fleunce.core.Speaker
import chat.fleunce.core.UsageSummary

@Composable
fun SettingsScreen(vm: FleunceViewModel, onExport: () -> Unit, onImport: () -> Unit, onReviewConsent: () -> Unit,
        ...
        LazyColumn(Modifier.weight(1f).testTag("settings-screen"), contentPadding = PaddingValues(start = 20.dp, end = 20.dp, bottom = 28.dp),
            ...
            item {
                SettingsGroup(stringResource(R.string.settings_just_your_pace),
                    ...
                    BasicTextField(value = prefs.interests, onValueChange = { vm.updatePreferences(prefs.copy(interests = it.take(500))) },
                        enabled = !vm.isRunning, textStyle = MaterialTheme.typography.bodyLarge.copy(color = FleunceColors.Ink),
                        cursorBrush = SolidColor(FleunceColors.Secondary), minLines = 1, maxLines = 4,
                        modifier = Modifier.fillMaxWidth().heightIn(min = 52.dp).padding(horizontal = 16.dp, vertical = 14.dp)
                            ...
                            if (prefs.interests.isEmpty()) Text(stringResource(R.string.settings_interests_label),
                                style = MaterialTheme.typography.bodyLarge, color = FleunceColors.Secondary.copy(alpha = .7f))
                            field()
                        })
                }
            }
                    ...
                    AnimatedVisibility(advanced, enter = expandVertically() + fadeIn(), exit = shrinkVertically() + fadeOut()) {
                        Column {
                            ...
                            if (vm.hasKey) Text(stringResource(R.string.settings_key_saved_notice), style = MaterialTheme.typography.bodySmall,
                                color = FleunceColors.Secondary, modifier = Modifier.padding(horizontal = 16.dp, vertical = 10.dp))
                            SettingsRow(stringResource(if (vm.hasKey) R.string.settings_replace_key else R.string.settings_save_key),
                                enabled = !vm.isRunning, tint = FleunceColors.Secondary, chevron = true, onClick = { keyDialog = true })
                            SettingsDivider()
                            SettingsRow(stringResource(R.string.settings_open_api_keys), tint = FleunceColors.Secondary,
                            ...
                            if (vm.hasKey) {
                                ...
                                SettingsRow(stringResource(R.string.settings_remove_key), enabled = !vm.isRunning,
                                    tint = FleunceColors.Red, onClick = { deleteKey = true })
                            }
                            Text(stringResource(R.string.settings_key_owner_footer), style = MaterialTheme.typography.bodySmall,
                                color = FleunceColors.Secondary, modifier = Modifier.padding(start = 16.dp, end = 16.dp, bottom = 16.dp))
                        }
                    }
                }
            }
            item {
                ...
                SettingsGroup(stringResource(R.string.settings_keep_comfortable), stringResource(R.string.settings_usage_footer)) {
                    ...
                    SettingsDivider()
                    SettingsRow(stringResource(R.string.settings_usage_billing_link), tint = FleunceColors.Secondary,
            ...
            item {
                SettingsGroup(stringResource(R.string.settings_section_your_data), stringResource(R.string.settings_backup_footer)) {
                    ...
                    SettingsRow(stringResource(R.string.settings_delete_all_data), enabled = !vm.isRunning,
                        tint = FleunceColors.Red, onClick = { deleteAll = true })
                }
            }
            item {
                SettingsGroup(stringResource(R.string.settings_section_help_privacy)) {
                    SettingsRow(stringResource(R.string.common_privacy_policy), tint = FleunceColors.Secondary,
                        onClick = { open("https://fleunce.chat/privacy/") })
                    SettingsDivider()
                    SettingsRow(stringResource(R.string.common_terms_of_use), tint = FleunceColors.Secondary,
                        onClick = { open("https://fleunce.chat/terms/") })
                    SettingsDivider()
                    SettingsRow(stringResource(R.string.common_contact_support), tint = FleunceColors.Secondary,
                        onClick = { open("https://fleunce.chat/support/") })
                    SettingsDivider()
            ...
            item {
                SettingsGroup {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        Text(stringResource(R.string.settings_app_version_footer, version), style = MaterialTheme.typography.bodySmall, color = FleunceColors.Secondary)
                        Text(stringResource(R.string.settings_models_footer), style = MaterialTheme.typography.bodySmall, color = FleunceColors.Secondary)
                    }
                    SettingsDivider()
                    SettingsRow(stringResource(R.string.settings_openai_data_controls), tint = FleunceColors.Secondary,
                    ...
                    Text(stringResource(R.string.settings_data_use_footer), style = MaterialTheme.typography.bodySmall,
                        color = FleunceColors.Secondary, modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp))
                    SettingsDivider()
    ...
    if (permissionDetails) AlertDialog(onDismissRequest = { permissionDetails = false },
        ...
        text = { Column(verticalArrangement = Arrangement.spacedBy(14.dp)) {
            ...
            Text(stringResource(R.string.settings_ai_permission_summary), color = FleunceColors.Secondary)
        } },
        confirmButton = {
            if (prefs.aiConsentVersion == AI_CONSENT_VERSION) FleunceTextButton(onClick = { permissionDetails = false; revokeConsent = true },
                enabled = !vm.isRunning, modifier = Modifier.testTag("revoke-ai-consent")) {
                Text(stringResource(R.string.settings_revoke_consent_button), color = FleunceColors.Red)
            } else FleunceTextButton(onClick = { permissionDetails = false; onReviewConsent() },
        ...
        }, dismissButton = { FleunceTextButton(onClick = { permissionDetails = false }) { Text(stringResource(R.string.common_close)) } })
...
@Composable
private fun SettingsHistorySheet(vm: FleunceViewModel, onDismiss: () -> Unit, onSelect: (SessionRecord) -> Unit, onDelete: (SessionRecord) -> Unit) {
        ...
        Column(Modifier.fillMaxHeight(.9f)) {
            ...
            LazyColumn(Modifier.weight(1f).testTag("settings-history-list"), contentPadding = PaddingValues(horizontal = 20.dp, vertical = 8.dp),
                ...
                if (vm.archive.sessions.isEmpty()) item {
                    Text(stringResource(R.string.history_empty), style = MaterialTheme.typography.bodyLarge, color = FleunceColors.Secondary,
                    ...
                    SettingsGroup {
                        Column(Modifier.fillMaxWidth().clickable { onSelect(session) }.padding(16.dp), verticalArrangement = Arrangement.spacedBy(5.dp)) {
                            ...
                            Text("${LanguageRegistry.get(session.languageID)?.name ?: session.languageID} · ${formatDate(session.startedAt)}",
                                color = FleunceColors.Secondary, style = MaterialTheme.typography.bodySmall)
                        }
                        SettingsDivider()
                        SettingsRow(stringResource(R.string.common_delete), enabled = !vm.isRunning, tint = FleunceColors.Red, onClick = { onDelete(session) })
                    }
                }
            }
        }
...
@Composable
private fun KeyDialog(vm: FleunceViewModel, onDismiss: () -> Unit) {
    ...
    Dialog(onDismissRequest = { key = ""; onDismiss() }) {
        ...
        Surface(shape = RoundedCornerShape(28.dp), color = FleunceColors.Surface) {
            Column(Modifier.padding(22.dp), verticalArrangement = Arrangement.spacedBy(15.dp)) {
                ...
                Text(stringResource(R.string.settings_key_dialog_note), color = FleunceColors.Secondary)
                FleunceTextField(
                    key,
                    { key = it.take(500) },
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.End) {
                    FleunceTextButton(onClick = { key = ""; onDismiss() }) { Text(stringResource(R.string.common_cancel)) }
...
private fun ConfirmDialog(title: String, message: String, confirm: String, onConfirm: () -> Unit, onDismiss: () -> Unit) {
    AlertDialog(
        ...
        title = { Text(title) },
        text = { Text(message) },
        confirmButton = { FleunceTextButton(onClick = onConfirm) { Text(confirm, color = FleunceColors.Red) } },
        dismissButton = { FleunceTextButton(onClick = onDismiss) { Text(stringResource(R.string.common_cancel)) } },
    )
}

@Composable
fun TranscriptDialog(vm: FleunceViewModel, session: SessionRecord, onDismiss: () -> Unit) {
        ...
        Surface(shape = RoundedCornerShape(28.dp), color = FleunceColors.Surface) {
            LazyColumn(Modifier.fillMaxWidth().padding(20.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
                item {
                    ...
                    Text("${formatDate(liveSession.startedAt)} · ${pluralStringResource(R.plurals.history_passages_count, liveSession.passages.size, liveSession.passages.size)}", color = FleunceColors.Secondary)
                }
                items(liveSession.passages, key = { it.id }) { passage ->
                    Column(
                        Modifier.fillMaxWidth().background(
                            if (passage.speaker == Speaker.user) FleunceColors.SurfaceBright else FleunceColors.Peach,
                            RoundedCornerShape(18.dp),
                        ).padding(15.dp),
                        verticalArrangement = Arrangement.spacedBy(7.dp),
                    ) {
                        Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                            Text(stringResource(if (passage.speaker == Speaker.user) R.string.history_speaker_you else R.string.history_speaker_fleunce),
                                style = MaterialTheme.typography.labelSmall, color = FleunceColors.Secondary, modifier = Modifier.weight(1f))
                        ...
                        if (passage.speaker == Speaker.user && !vm.isRunning) FleunceTextButton(onClick = { correcting = passage }) { Text(stringResource(R.string.history_edit_passage_button)) }
                    }
                }
                liveSession.topics.flatMap { it.sources }.filter { it.safeUrl() != null }.takeIf { it.isNotEmpty() }?.let { sources ->
                    ...
                    items(sources) { source ->
                        val uriHandler = LocalUriHandler.current
                        Text("↗ ${source.title}", color = FleunceColors.Orange, modifier = Modifier.clickable { source.safeUrl()?.let(uriHandler::openUri) }.padding(vertical = 6.dp))
                    }
                }
                item { FleunceTextButton(onClick = onDismiss, modifier = Modifier.fillMaxWidth()) { Text(stringResource(R.string.common_close)) } }
            }
        }
    }
...
private fun CorrectionDialog(passage: Passage, onSave: (String) -> Unit, onDismiss: () -> Unit) {
    ...
    AlertDialog(
        ...
        text = { FleunceTextField(text, { text = it.take(10_000) }, minLines = 3, maxLines = 9) },
