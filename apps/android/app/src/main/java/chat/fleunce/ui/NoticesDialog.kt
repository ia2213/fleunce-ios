...
private val NOTICE_FILES = listOf("Nunito-OFL.txt", "THIRD-PARTY-NOTICES.txt", "Fleunce-LICENSE.txt", "Apache-2.0.txt", "WebRTC-SDK-LICENSE.txt", "WebRTC-THIRD-PARTY-NOTICES.md")
        ...
        Surface(Modifier.fillMaxSize(), color = FleunceColors.Cream) {
            Column {
                Row(Modifier.padding(start = 20.dp, end = 8.dp, top = 12.dp), verticalAlignment = Alignment.CenterVertically) {
                    ...
                    FleunceTextButton(onClick = onDismiss) { Text(stringResource(R.string.common_close)) }
                }
                LazyColumn(Modifier.fillMaxSize().testTag("notices-list"), contentPadding = PaddingValues(20.dp)) {
                    items(blocks) { block ->
                        if (block.isFileTitle) {
                            ...
                            Text(block.text, style = MaterialTheme.typography.bodySmall, color = FleunceColors.Secondary, modifier = Modifier.padding(bottom = 10.dp))
                        }
                    }
                }
            }
