...
import chat.fleunce.core.ConversationTheme
import chat.fleunce.core.TopicBrief
import chat.fleunce.core.WordState
import java.time.Instant
import java.time.ZoneId
import java.time.format.DateTimeFormatter

@Composable
fun TopicsScreen(vm: FleunceViewModel, onChoose: () -> Unit, onCurrentTopic: (String) -> Unit) {
    ...
    LazyVerticalGrid(columns = GridCells.Adaptive(if (LocalDensity.current.fontScale > 1.3f) 260.dp else 150.dp),
        ...
        item(span = { GridItemSpan(maxLineSpan) }) {
            FleunceSearchField(search, { search = it.take(80) }, stringResource(R.string.topics_search_placeholder))
        }
        item(span = { GridItemSpan(maxLineSpan) }) {
            ...
            LazyRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                items(categories) { item ->
                    FilterChip(selected = category == item, onClick = { category = item }, label = { Text(categoryLabel(item), style = MaterialTheme.typography.bodySmall) },
                        ...
                        colors = FilterChipDefaults.filterChipColors(containerColor = FleunceColors.Surface.copy(alpha = .70f),
                            selectedContainerColor = FleunceColors.Peach, selectedLabelColor = FleunceColors.Ink),
        ...
        if (themes.isEmpty()) item(span = { GridItemSpan(maxLineSpan) }) {
            Text(stringResource(R.string.topics_no_results), color = FleunceColors.Secondary, modifier = Modifier.padding(vertical = 28.dp))
        }
    }
...
private fun ThemeCard(theme: ConversationTheme, index: Int, onClick: () -> Unit) {
    Surface(
        ...
        color = FleunceColors.Panels.getOrElse(theme.colorIndex) { FleunceColors.Panels[index % FleunceColors.Panels.size] },
    ) {
        Column(Modifier.fillMaxWidth().heightIn(min = 180.dp).padding(19.dp), verticalArrangement = Arrangement.spacedBy(28.dp)) {
            FleunceIcon(themeSymbol(theme.id), Modifier.size(31.dp), color = FleunceColors.Secondary)
            Column(verticalArrangement = Arrangement.spacedBy(5.dp)) {
                Text(theme.title, style = MaterialTheme.typography.titleMedium)
                Text(theme.subtitle, color = FleunceColors.Secondary, style = MaterialTheme.typography.bodySmall)
            }
        }
    }
}
                ...
                vm.topicResult?.let { brief ->
                    ...
                    item {
                        Button(
                            ...
                            modifier = Modifier.fillMaxWidth(),
                            colors = ButtonDefaults.buttonColors(containerColor = FleunceColors.Orange, contentColor = FleunceColors.Ink),
                        ) { Text(stringResource(R.string.topics_talk_about_button)) }
                    }
                }
                item { Text(stringResource(R.string.topics_sources_note), style = MaterialTheme.typography.bodySmall, color = FleunceColors.Secondary) }
...
@Composable
fun WordsScreen(vm: FleunceViewModel) {
    var search by rememberSaveable { mutableStateOf("") }
    ...
    LazyColumn(
        ...
        item {
            FleunceSearchField(search, { search = it.take(80) }, stringResource(R.string.words_search_placeholder))
        }
        if (words.isEmpty()) item {
            Surface(shape = RoundedCornerShape(28.dp), color = FleunceColors.Sage) {
                Column(Modifier.padding(24.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    FleunceIcon(FleunceSymbol.Leaf, Modifier.size(34.dp), color = FleunceColors.Secondary)
            ...
            Surface(Modifier.fillMaxWidth().clickable { selected = word }, shape = RoundedCornerShape(20.dp), color = FleunceColors.Surface) {
                Row(Modifier.padding(18.dp), verticalAlignment = Alignment.CenterVertically) {
                    Column(Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                        Text(word.lemma, style = MaterialTheme.typography.titleLarge)
                        Text(word.meaning, color = FleunceColors.Secondary)
                    }
                    Column(horizontalAlignment = Alignment.End, verticalArrangement = Arrangement.spacedBy(7.dp)) {
                        RecallBars(word.bars)
                        Text(wordLabel(word.label), style = MaterialTheme.typography.labelSmall, color = FleunceColors.Secondary)
                    }
                }
            }
        }
        item {
            Text(stringResource(R.string.words_recall_note), style = MaterialTheme.typography.bodySmall, color = FleunceColors.Secondary)
        }
        if (vm.learner.capabilities.isNotEmpty()) item {
            Surface(shape = RoundedCornerShape(24.dp), color = FleunceColors.Butter) {
                Column(Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
                    ...
                    Text(stringResource(R.string.words_capabilities_note), style = MaterialTheme.typography.bodySmall, color = FleunceColors.Secondary)
                }
            }
        }
    ...
    Dialog(onDismissRequest = onDismiss) {
        Surface(shape = RoundedCornerShape(28.dp), color = FleunceColors.Surface) {
            Column(Modifier.padding(24.dp), verticalArrangement = Arrangement.spacedBy(15.dp)) {
                ...
                Text(word.meaning, style = MaterialTheme.typography.titleLarge, color = FleunceColors.Secondary)
                Row(horizontalArrangement = Arrangement.spacedBy(10.dp), verticalAlignment = Alignment.CenterVertically) {
                    RecallBars(word.bars); Text(wordLabel(word.label))
                }
                Text(stringResource(wordExplanationRes(word)))
                Surface(shape = RoundedCornerShape(18.dp), color = FleunceColors.Peach) { Text(stringResource(R.string.words_example_quote, word.example), Modifier.padding(17.dp)) }
                Text(
                ...
                FleunceTextButton(onClick = onRemove) { Text(stringResource(R.string.words_remove_button), color = FleunceColors.Red) }
                FleunceTextButton(onClick = onDismiss, modifier = Modifier.align(Alignment.End)) { Text(stringResource(R.string.words_done_button)) }
            }
        }
    }
}
...
internal fun formatDate(secondsSince2001: Double): String = runCatching {
    val unix = (secondsSince2001 + chat.fleunce.core.APPLE_EPOCH_UNIX_SECONDS).toLong()
...
private fun themeSymbol(id: String) = when (id) {
    "coffee" -> FleunceSymbol.Coffee
    "weekend" -> FleunceSymbol.Sun
    "walk", "cabin" -> FleunceSymbol.Leaf
    "dinner", "restaurant" -> FleunceSymbol.Food
    "music" -> FleunceSymbol.Music
    "books" -> FleunceSymbol.Words
    "today", "travel", "travelstories" -> FleunceSymbol.Globe
    else -> FleunceSymbol.Sparkles
}
