...
class FleunceViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun handle(event: JsonObject) {
        ...
        when (event["type"]?.jsonPrimitive?.content) {
            "fleunce.session.created" -> updateSession { it.providerID = (event["session"] as? JsonObject)?.get("id")?.jsonPrimitive?.content; it.voiceSeconds = 15.0 }
