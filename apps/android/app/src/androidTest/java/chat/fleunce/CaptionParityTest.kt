...
class CaptionParityTest {
    ...
    @Before fun prepare() {
        assertEquals("chat.fleunce.android.uitest", InstrumentationRegistry.getInstrumentation().targetContext.packageName)
        vm = compose.awaitHistoryLoaded()
        compose.runOnIdle {
            original = ArchiveCodec.encode(vm.archive); originalHasKey = vm.hasKey
            val field = FleunceViewModel::class.java.getDeclaredField("api").apply { isAccessible = true }
    ...
    @After fun restore() {
        ...
        compose.runOnIdle {
            ...
            FleunceViewModel::class.java.getDeclaredField("voiceSession").apply { isAccessible = true }.setBoolean(vm, false)
            FleunceViewModel::class.java.getDeclaredField("api").apply { isAccessible = true }.set(vm, originalAPI)
    ...
    private fun startRecording(language: String) {
        ...
        recorder = InstrumentationRegistry.getInstrumentation().uiAutomation.executeShellCommand(
            "screenrecord --size 720x1616 --bit-rate 4000000 --time-limit 90 /sdcard/Download/fleunce-$language.mp4")
        Thread.sleep(1500)
    }
    ...
    @Test fun longMeaningRequestKeepsEveryCharacterAndCachesTheFullRevision() {
        ...
        requests.clear()
        compose.runOnIdle {
            FleunceViewModel::class.java.getDeclaredMethod("scheduleTranslation", Boolean::class.javaPrimitiveType)
    ...
    @Test fun oversizedHostedMeaningShowsLimitWithoutDispatchOrRetry() {
        ...
        requests.clear()
        compose.runOnIdle {
            val hosted = FleunceViewModel::class.java.getDeclaredField("hostedSessionIDs").apply { isAccessible = true }
        ...
        compose.runOnIdle {
            FleunceViewModel::class.java.getDeclaredField("hostedSessionIDs").apply { isAccessible = true }.set(vm, emptySet<String>())
        }
        show("zh", "你好。", "")
        compose.runOnIdle {
            FleunceViewModel::class.java.getDeclaredMethod("scheduleTranslation", Boolean::class.javaPrimitiveType)
    ...
    @Test fun longMandarinReplyKeepsBothCaptionsVisibleAndEveryReadingReachable() {
        ...
        android.util.Log.i("FleunceCaptionCheck", "Long caption: render")
        ...
        android.util.Log.i("FleunceCaptionCheck", "Long caption: capture")
        capture("mandarin-long-caption")
        android.util.Log.i("FleunceCaptionCheck", "Long caption: scroll reading")
        ...
        compose.onNodeWithTag("start-conversation").assertIsDisplayed()
        compose.onNodeWithTag("floating-navigation").assertIsDisplayed()
        android.util.Log.i("FleunceCaptionCheck", "Long caption: verified")
    }
}
