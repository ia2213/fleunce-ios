...
class NativeCompatibilityTest {
    @Test fun keystoreEncryptsAndDeletesOnlyIsolatedTestCredentials() {
        ...
        val store = CredentialStore(context, "fleunce_test_$namespace", "chat.fleunce.test.$namespace")
        val fake = "sk-offline-test-credential-never-sent"
        try {
            store.save(fake)
            val raw = context.getSharedPreferences("fleunce_test_$namespace", Context.MODE_PRIVATE).all.values.joinToString()
            assertFalse(raw.contains(fake))
            assertTrue(store.hasKey)
            assertEquals(fake, CredentialStore(context, "fleunce_test_$namespace", "chat.fleunce.test.$namespace").read())
