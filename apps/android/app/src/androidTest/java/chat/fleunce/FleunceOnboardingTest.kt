...
class FleunceOnboardingTest {
    ...
    fun firstRun_requiresBothLanguageStepsAndExplicitAiConsent() {
        ...
        compose.onNodeWithTag("ai-consent-title").assertIsDisplayed()
        compose.runOnIdle {
            assertEquals(CloudAction.StartVoice, ViewModelProvider(compose.activity)[FleunceViewModel::class.java].pendingCloudAction)
        }
        compose.onNodeWithTag("ai-consent-decline").performClick()
        compose.runOnIdle { assertNull(ViewModelProvider(compose.activity)[FleunceViewModel::class.java].pendingCloudAction) }
