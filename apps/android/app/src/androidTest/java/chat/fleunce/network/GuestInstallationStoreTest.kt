...
class GuestInstallationStoreTest {
    @Test fun guestIdentityAndTransferTicketSurviveRecreationWithoutEnteringMemberStorage() = runBlocking {
        ...
        try {
            ...
            val raw = context.getSharedPreferences("fleunce_guest_installation", Context.MODE_PRIVATE).all.values.joinToString()
    ...
    @Test fun wrongOriginAndCipherCorruptionFailClosedWithoutReplacingInstallation() = runBlocking {
        ...
        try {
            ...
            context.getSharedPreferences("fleunce_guest_installation", Context.MODE_PRIVATE).edit().putString("ciphertext", "invalid").commit()
    ...
    private fun cleanup(context: Context) {
        context.getSharedPreferences("fleunce_guest_installation", Context.MODE_PRIVATE).edit().clear().commit()
        KeyStore.getInstance("AndroidKeyStore").apply { load(null); deleteEntry("chat.fleunce.guest.aes") }
    }
}
