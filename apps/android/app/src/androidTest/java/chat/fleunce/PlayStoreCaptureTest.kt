...
/** Listing artwork rendered from the same native Brand and FleunceOrb as the app. */
@RunWith(AndroidJUnit4::class)
    ...
    @Test fun renderFeatureGraphic() {
        compose.setContent {
            CompositionLocalProvider(LocalDensity provides Density(1f, 1f)) {
                FleunceTheme {
                    Box(Modifier.requiredSize(1024.dp, 500.dp).background(FleunceColors.Cream).testTag("play-feature")) {
                        Column(Modifier.offset(80.dp, 96.dp)) {
                            Brand()
                            Spacer(Modifier.height(35.dp))
                            Text("It starts with\na hello.", color = FleunceColors.Ink,
                                style = TextStyle(fontFamily = FleunceRounded, fontSize = 57.sp, lineHeight = 65.sp,
                                    fontWeight = FontWeight.SemiBold, letterSpacing = (-1.5).sp))
                            Spacer(Modifier.height(22.dp))
                            Text("Learn by talking.", color = FleunceColors.Secondary,
                                style = TextStyle(fontFamily = FleunceRounded, fontSize = 23.sp))
                        }
                        FleunceOrb(modifier = Modifier.offset(634.dp, 110.dp).size(280.dp))
                    }
                }
            }
        }
